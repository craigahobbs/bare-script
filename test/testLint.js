...
import {lintScript} from '../lib/lint.js';
import test from 'node:test';
import {validateScript} from '../lib/model.js';
...
test('lintScript, global unknown global inherited name', () => {
    const script = {
        'scriptName': 'test.bare',
        'scriptLines': [
            'a = constructor'
        ...
        'statements': [
            {'expr': {'name': 'a', 'expr': {'variable': 'constructor'}, 'lineNumber': 1}}
        ]
    };
    assert.deepEqual(lintScript(validateScript(script), {}), [
        'test.bare:1: Unknown global variable "constructor"'
    ]);
});


test('lintScript, function call inherited name', () => {
    const script = {
        'scriptName': 'test.bare',
        'scriptLines': [
            'function testFn():',
        ...
        'statements': [
            {
                'function': {
                    'name': 'testFn',
                    'statements': [
                        {'return': {'expr': {'function': {'name': 'constructor'}}, 'lineNumber': 2}}
                    ],
                    'lineNumber': 1
                }
            }
    ...
    assert.deepEqual(lintScript(validateScript(script), {}), [
        'test.bare:2: Unknown global variable "constructor"'
    ]);
});


test('lintScript, function unused variable proto name', () => {
    const script = {
        'scriptName': 'test.bare',
        'scriptLines': [
            'function testFn():',
        ...
        'statements': [
            {
                'function': {
                    'name': 'testFn',
                    'statements': [
                        {'expr': {'name': '__proto__', 'expr': {'number': 1}, 'lineNumber': 2}},
                        {'return': {'expr': {'number': 2}, 'lineNumber': 3}}
                    ],
                    'lineNumber': 1
                }
            }
    ...
    assert.deepEqual(lintScript(validateScript(script)), [
        'test.bare:2: Unused variable "__proto__" defined in function "testFn"'
    ]);
});
