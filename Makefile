...
lib/includeSource.js: $(sort $(wildcard lib/include/*.bare)) | build/npm.build
	$(NODE_SHELL) node --input-type=module -e "$$INCLUDE_SOURCE_JS" $@ $^
...
for (const includePath of includePaths) {
    ...
    const includeTextEscaped = includeText.replace(/\\/g, '\\\\').replace(/\`/g, '\\`').replace(/\$$\{/g, '\\$${');
    parts.push(`\n\n// $${includeName}.bare\nexport const $${exportName} = \`$${includeTextEscaped}\`;\n`);
