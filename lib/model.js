...
function getVariableAssignmentsAndUses(statements, assigns, uses) {
    for (const [ixStatement, statement] of statements.entries()) {
        const expr = (statement.expr ?? null);
        const jump = (statement.jump ?? null);
        const ret = (statement.return ?? null);
        if (expr !== null && 'name' in expr) {
            if (!(expr.name in assigns)) {
                assigns[expr.name] = ixStatement;
            }
            getExpressionVariableUses(expr.expr, uses, ixStatement);
        } else if (jump !== null && 'expr' in jump) {
            getExpressionVariableUses(jump.expr, uses, ixStatement);
        } else if (ret !== null && 'expr' in ret) {
            getExpressionVariableUses(ret.expr, uses, ixStatement);
        }
    }
}
...
function getExpressionVariableUses(expr, uses, ixStatement) {
    const varName = (expr.variable ?? null);
    const binary = (expr.binary ?? null);
    const unary = (expr.unary ?? null);
    const groupExpr = (expr.group ?? null);
    if (varName !== null) {
        if (!(varName in uses)) {
            uses[varName] = ixStatement;
        }
    } else if (binary !== null) {
        getExpressionVariableUses(binary.left, uses, ixStatement);
        getExpressionVariableUses(binary.right, uses, ixStatement);
    } else if (unary !== null) {
        getExpressionVariableUses(unary.expr, uses, ixStatement);
    } else if (groupExpr !== null) {
        getExpressionVariableUses(groupExpr, uses, ixStatement);
    }
}
