...
export function valueArgsValidate(fnArgs, args, errorReturnValue = null) {
    ...
    for (let ix = 0; ix < fnArgsLength; ix++) {
        ...
        if (argType === 'number') {
            ...
            if (typeof argLT !== 'undefined' && !(argValue < argLT)) {
                throw new ValueArgsError(fnArg.name, argValue, errorReturnValue);
            }

            // Less-than-or-equal-to?
            const argLTE = fnArg.lte;
            if (typeof argLTE !== 'undefined' && !(argValue <= argLTE)) {
                throw new ValueArgsError(fnArg.name, argValue, errorReturnValue);
            }

            // Greater-than?
            const argGT = fnArg.gt;
            if (typeof argGT !== 'undefined' && !(argValue > argGT)) {
                throw new ValueArgsError(fnArg.name, argValue, errorReturnValue);
            }

            // Greater-than-or-equal-to?
            const argGTE = fnArg.gte;
            if (typeof argGTE !== 'undefined' && !(argValue >= argGTE)) {
                throw new ValueArgsError(fnArg.name, argValue, errorReturnValue);
            }
        }
    }
