...
export function valueArgsValidate(fnName, fnArgs, args, errorReturnValue = null) {
    ...
    for (const [ix, fnArg] of fnArgs.entries()) {
        ...
        if (ix >= argsLength) {
            ...
            // Argument nullable?
            if (!argType || argNullable) {
                args.push(null);
                continue;
            }

            // Invalid null value...
            valueArgsError(fnName, argName, null, errorReturnValue);
        }
        ...
        if (argValue === null) {
            // Argument nullable?
            if (argType && !argNullable) {
                valueArgsError(fnName, argName, argValue, errorReturnValue);
            }
            continue;
        }
        ...
        // Integer number?
        if (argInteger && argType === 'number' && argValue !== null && Math.floor(argValue) !== argValue) {
            valueArgsError(fnName, argName, argValue, errorReturnValue);
        }
 ...
 */
export function valueArgsError(fnName, argName, argValue, returnValue = null) {
