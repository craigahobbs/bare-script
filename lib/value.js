 ...
 */
export function valueArgsValidate(fnName, fnArgs, args) {
    ...
    for (let ix = 0; ix < fnArgsLength; ix++) {
        ...
        if (ix >= argsLength) {
            ...
            // Argument nullable?
            if (!fnArg.type || fnArg.nullable) {
                args.push(null);
                continue;
            }

            // Invalid null value...
            valueArgsError(fnName, fnArg.name, null);
        }
        ...
        if ((argType === 'number' && argValueType !== 'number') ||
            ...
            valueArgsError(fnName, fnArg.name, argValue);
        }

        // Integer number?
        if (fnArg.integer && argType === 'number' && argValue !== null && Math.floor(argValue) !== argValue) {
            valueArgsError(fnName, fnArg.name, argValue);
        }

        // Less-than?
        const ltValue = fnArg.lt;
        if (typeof ltValue !== 'undefined' && !(valueCompare(argValue, ltValue) < 0)) {
            valueArgsError(fnName, fnArg.name, argValue);
        }

        // Less-than-or-equal-to?
        const lteValue = fnArg.lte;
        if (typeof lteValue !== 'undefined' && !(valueCompare(argValue, lteValue) <= 0)) {
            valueArgsError(fnName, fnArg.name, argValue);
        }

        // Greater-than?
        const gtValue = fnArg.gt;
        if (typeof gtValue !== 'undefined' && !(valueCompare(argValue, gtValue) > 0)) {
            valueArgsError(fnName, fnArg.name, argValue);
        }

        // Greater-than-or-equal-to?
        const gteValue = fnArg.gte;
        if (typeof gteValue !== 'undefined' && !(valueCompare(argValue, gteValue) >= 0)) {
            valueArgsError(fnName, fnArg.name, argValue);
        }
    }
    return args;
 ...
 */
export function valueArgsError(fnName, argName, argValue) {
    throw Error(`Function "${fnName}" called with invalid "${argName}" argument value, ${valueJSON(argValue)}`);
}
...
export function valueArgsModel(fnArgs) {
    return validateType(valueArgsTypes, 'FunctionArguments', fnArgs);
}
...
struct FunctionArgument
    ...
    # If true, the number argument is an integer
