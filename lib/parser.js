...
const rScriptAssignment = /^\s*(?:(?<async>async)\s+)?(?<name>[A-Za-z_]\w*)\s*=\s*(?<expr>.*)$/;
...
const rScriptExpr = /^\s*(?:(?<async>async)\s+)?(?:(?<return>return)\s+)?(?<expr>.*?)\s*$/;
...
export function parseScript(scriptText) {
    ...
    for (const linePart of lines) {
        ...
        if (matchAssignment !== null) {
            ...
            if (matchAssignment.groups.async === 'async') {
                assignStatement.assignment.async = true;
            }
        ...
        if (matchExpr.groups.async === 'async') {
            exprStatement.expression.async = true;
        }
