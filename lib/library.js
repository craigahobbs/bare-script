...
function regexMatchAll(args) {
    ...
    return Array.from(string.matchAll(regexEnsureGlobal(regex)), regexMatchGroups);
}
...
// Helper to re-compile a regex with the global flag, cached by flags and source
function regexEnsureGlobal(regex) {
    ...
    let regexGlobal = regexGlobalCache.get(cacheKey) ?? null;
    if (regexGlobal === null) {
        regexGlobal = new RegExp(source, `${flags}g`);
        regexGlobalCache.set(cacheKey, regexGlobal);
    }
    return regexGlobal;
}

const regexGlobalCache = new Map();
