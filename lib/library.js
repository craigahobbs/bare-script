...
function arrayGet(args) {
    ...
    if (index >= array.length) {
        valueArgsError('arrayGet', 'index', index);
    }
    return array[index];
}
...
function arrayIndexOf(args, options) {
    ...
    if (index >= array.length) {
        valueArgsError('arrayIndexOf', 'index', index, -1);
    }
...
function arrayLastIndexOf(args, options) {
    ...
    if (index >= array.length) {
        valueArgsError('arrayLastIndexOf', 'index', index, -1);
    }
...
function arrayPush([array = null, ...values]) {
    if (valueType(array) !== 'array') {
        valueArgsError('arrayPush', 'array', array);
    }
...
function arraySet(args) {
    ...
    if (index >= array.length) {
        valueArgsError('arraySet', 'index', index);
    }
...
function arraySlice(args) {
    ...
    if (start > array.length) {
        valueArgsError('arraySlice', 'start', start);
    }
    if (end > array.length) {
        valueArgsError('arraySlice', 'end', end);
    }
...
function dataParseCSV(args) {
    ...
    for (const arg of args) {
        ...
        if (valueType(arg) !== 'string') {
            valueArgsError('dataParseCSV', 'text', arg);
        }
        lines.push(arg);
    }
...
function mathLog(args) {
    ...
    if (base === 1) {
        valueArgsError('mathLog', 'base', base);
    }
