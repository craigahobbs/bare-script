...
import {
    ...
    valueParseDatetime, valueParseInteger, valueParseNumber, valueString, valueType
} from './value.js';
...
function arrayCopy(args) {
    const [array] = valueArgsValidate('arrayCopy', arrayCopyArgs, args);
    return [...array];
}
...
function arrayExtend(args) {
    const [array, array2] = valueArgsValidate('arrayExtend', arrayExtendArgs, args);
    array.push(...array2);
    return array;
}
...
function arrayGet(args) {
    const [array, index] = valueArgsValidate('arrayGet', arrayGetArgs, args);
    if (index >= array.length) {
        valueArgsError('index', index);
    }
    return array[index];
}
...
function arrayIndexOf(args, options) {
    const [array, value, index] = valueArgsValidate('arrayIndexOf', arrayIndexOfArgs, args, -1);
    if (index >= array.length) {
        valueArgsError('index', index, -1);
    }
...
function arrayJoin(args) {
    const [array , separator] = valueArgsValidate('arrayJoin', arrayJoinArgs, args);
...
function arrayLastIndexOf(args, options) {
    const [array, value, indexArg] = valueArgsValidate('arrayLastIndexOf', arrayLastIndexOfArgs, args, -1);
    let index = indexArg;
    if (index === null) {
...
function arrayLength(args) {
    const [array] = valueArgsValidate('arrayLength', arrayLengthArgs, args, 0);
    return array.length;
}
...
function arrayNewSize(args) {
    const [size, value] = valueArgsValidate('arrayNewSize', arrayNewSizeArgs, args);
...
function arrayPop(args) {
    const [array] = valueArgsValidate('arrayPop', arrayPopArgs, args);
    if (array.length === 0) {
        return null;
    }
    return array.pop();
...
function arrayPush(args) {
    const [array, values] = valueArgsValidate('arrayPush', arrayPushArgs, args);
    array.push(...values);
    return array;
}
...
function arraySet(args) {
    const [array, index, value] = valueArgsValidate('arraySet', arraySetArgs, args);
    if (index >= array.length) {
        valueArgsError('index', index);
    }
...
function arrayShift(args) {
    const [array] = valueArgsValidate('arrayShift', arrayShiftArgs, args);
    if (array.length === 0) {
        return null;
    }
    return array.shift();
...
function arraySlice(args) {
    const [array, start, endArg] = valueArgsValidate('arraySlice', arraySliceArgs, args);
    const end = endArg !== null ? endArg : array.length;
    if (start > array.length) {
        valueArgsError('start', start);
    }
    if (end > array.length) {
        valueArgsError('end', end);
    }
...
function arraySort(args, options) {
    const [array, compareFn] = valueArgsValidate('arraySort', arraySortArgs, args);
    if (compareFn === null) {
...
function dataAggregate(args) {
    const [data, aggregation] = valueArgsValidate('dataAggregate', dataAggregateArgs, args);
...
function dataCalculatedField(args, options) {
    const [data, fieldName, expr, variables] = valueArgsValidate('dataCalculatedField', dataCalculatedFieldArgs, args);
...
function dataFilter(args, options) {
    const [data, expr, variables] = valueArgsValidate('dataFilter', dataFilterArgs, args);
...
function dataJoin(args, options) {
    const [leftData, rightData, joinExpr, rightExpr, isLeftJoin, variables] = valueArgsValidate('dataJoin', dataJoinArgs, args);
...
function dataSort(args) {
    const [data, sorts] = valueArgsValidate('dataSort', dataSortArgs, args);
...
function dataTop(args) {
    const [data, count, categoryFields] = valueArgsValidate('dataTop', dataTopArgs, args);
...
function dataValidate(args) {
    const [data, csv] = valueArgsValidate('dataValidate', dataValidateArgs, args);
...
function datetimeDay(args) {
    const [datetime] = valueArgsValidate('datetimeDay', datetimeDayArgs, args);
...
function datetimeHour(args) {
    const [datetime] = valueArgsValidate('datetimeHour', datetimeHourArgs, args);
...
function datetimeISOFormat(args) {
    const [datetime, isDate] = valueArgsValidate('datetimeISOFormat', datetimeISOFormatArgs, args);
...
function datetimeISOParse(args) {
    const [string] = valueArgsValidate('datetimeISOParse', datetimeISOParseArgs, args);
...
function datetimeMillisecond(args) {
    const [datetime] = valueArgsValidate('datetimeMillisecond', datetimeMillisecondArgs, args);
...
function datetimeMinute(args) {
    const [datetime] = valueArgsValidate('datetimeMinute', datetimeMinuteArgs, args);
...
function datetimeMonth(args) {
    const [datetime] = valueArgsValidate('datetimeMonth', datetimeMonthArgs, args);
...
function datetimeNew(args) {
    const [year, month, day, hour, minute, second, millisecond] = valueArgsValidate('datetimeNew', datetimeNewArgs, args);
...
function datetimeSecond(args) {
    const [datetime] = valueArgsValidate('datetimeSecond', datetimeSecondArgs, args);
...
function datetimeYear(args) {
    const [datetime] = valueArgsValidate('datetimeYear', datetimeYearArgs, args);
...
function jsonParse(args) {
    const [string] = valueArgsValidate('jsonParse', jsonParseArgs, args);
...
function jsonStringify(args) {
    const [value, indent] = valueArgsValidate('jsonStringify', jsonStringifyArgs, args);
...
function mathAbs(args) {
    const [x] = valueArgsValidate('mathAbs', mathAbsArgs, args);
    return Math.abs(x);
}
...
function mathAcos(args) {
    const [x] = valueArgsValidate('mathAcos', mathAcosArgs, args);
    return Math.acos(x);
}
...
function mathAsin(args) {
    const [x] = valueArgsValidate('mathAsin', mathAsinArgs, args);
    return Math.asin(x);
}
...
function mathAtan(args) {
    const [x] = valueArgsValidate('mathAtan', mathAtanArgs, args);
    return Math.atan(x);
}
...
function mathAtan2(args) {
    const [y, x] = valueArgsValidate('mathAtan2', mathAtan2Args, args);
...
function mathCeil(args) {
    const [x] = valueArgsValidate('mathCeil', mathCeilArgs, args);
    return Math.ceil(x);
}
...
function mathCos(args) {
    const [x] = valueArgsValidate('mathCos', mathCosArgs, args);
    return Math.cos(x);
}
...
function mathFloor(args) {
    const [x] = valueArgsValidate('mathFloor', mathFloorArgs, args);
    return Math.floor(x);
}
...
function mathLn(args) {
    const [x] = valueArgsValidate('mathLn', mathLnArgs, args);
    return Math.log(x);
}
...
function mathLog(args) {
    const [x, base] = valueArgsValidate('mathLog', mathLogArgs, args);
    if (base === 1) {
        valueArgsError('base', base);
    }
...
function mathRound(args) {
    const [x, digits] = valueArgsValidate('mathRound', mathRoundArgs, args);
...
function mathSign(args) {
    const [x] = valueArgsValidate('mathSign', mathSignArgs, args);
    return Math.sign(x);
}
...
function mathSin(args) {
    const [x] = valueArgsValidate('mathSin', mathSinArgs, args);
    return Math.sin(x);
}
...
function mathSqrt(args) {
    const [x] = valueArgsValidate('mathSqrt', mathSqrtArgs, args);
    return Math.sqrt(x);
}
...
function mathTan(args) {
    const [x] = valueArgsValidate('mathTan', mathTanArgs, args);
    return Math.tan(x);
}
...
function numberParseFloat(args) {
    const [string] = valueArgsValidate('numberParseFloat', numberParseFloatArgs, args);
...
function numberParseInt(args) {
    const [string, radix] = valueArgsValidate('numberParseInt', numberParseIntArgs, args);
...
function numberToFixed(args) {
    const [x, digits, trim] = valueArgsValidate('numberToFixed', numberToFixedArgs, args);
...
function objectAssign(args) {
    const [object, object2] = valueArgsValidate('objectAssign', objectAssignArgs, args);
...
function objectCopy(args) {
    const [object] = valueArgsValidate('objectCopy', objectCopyArgs, args);
    return {...object};
}
...
function objectDelete(args) {
    const [object, key] = valueArgsValidate('objectDelete', objectDeleteArgs, args);
    delete object[key];
    return null;
}
...
function objectGet(args) {
    ...
    const [object, key, defaultValue] = valueArgsValidate('objectGet', objectGetArgs, args, defaultValueArg);
...
function objectHas(args) {
    const [object, key] = valueArgsValidate('objectHas', objectHasArgs, args, false);
...
function objectKeys(args) {
    const [object] = valueArgsValidate('objectKeys', objectKeysArgs, args);
...
function objectSet(args) {
    const [object, key, value] = valueArgsValidate('objectSet', objectSetArgs, args);
    object[key] = value;
    return value;
}
...
function regexEscape(args) {
    const [string] = valueArgsValidate('regexEscape', regexEscapeArgs, args);
...
function regexMatch(args) {
    const [regex, string] = valueArgsValidate('regexMatch', regexMatchArgs, args);
...
function regexMatchAll(args) {
    const [regex, string] = valueArgsValidate('regexMatchAll', regexMatchAllArgs, args);
...
function regexNew(args) {
    const [pattern, flags] = valueArgsValidate('regexNew', regexNewArgs, args);
...
function regexReplace(args) {
    const [regex, string, substr] = valueArgsValidate('regexReplace', regexReplaceArgs, args);
...
function regexSplit(args) {
    const [regex, string] = valueArgsValidate('regexSplit', regexSplitArgs, args);
...
function schemaParseEx(args) {
    const [lines, typesArg, filename] = valueArgsValidate('schemaParseEx', schemaParseExArgs, args);
    const linesType = valueType(lines);
    const types = typesArg !== null ? typesArg : {};
    if (linesType !== 'array' && linesType !== 'string') {
        valueArgsError('lines', lines);
    }
...
function schemaValidate(args) {
    const [types, typeName, value] = valueArgsValidate('schemaValidate', schemaValidateArgs, args);
...
function schemaValidateTypeModel(args) {
    const [types] = valueArgsValidate('schemaValidateTypeModel', schemaValidateTypeModelArgs, args);
...
function stringCharCodeAt(args) {
    const [string, index] = valueArgsValidate('stringCharCodeAt', stringCharCodeAtArgs, args);
...
function stringEndsWith(args) {
    const [string, search] = valueArgsValidate('stringEndsWith', stringEndsWithArgs, args);
...
function stringFromCharCode(charCodes) {
    for (const charCode of charCodes) {
        if (valueType(charCode) !== 'number' || Math.floor(charCode) !== charCode || charCode < 0) {
            valueArgsError('charCodes', charCode);
        }
    }
...
function stringIndexOf(args) {
    const [string, search, index] = valueArgsValidate('stringIndexOf', stringIndexOfArgs, args, -1);
    if (index >= string.length) {
        valueArgsError('index', index, -1);
    }
...
function stringLastIndexOf(args) {
    const [string, search, indexArg] = valueArgsValidate('stringLastIndexOf', stringLastIndexOfArgs, args, -1);
...
function stringLength(args) {
    const [string] = valueArgsValidate('stringLength', stringLengthArgs, args, 0);
...
function stringLower(args) {
    const [string] = valueArgsValidate('stringLower', stringLowerArgs, args);
...
function stringRepeat(args) {
    const [string, count] = valueArgsValidate('stringRepeat', stringRepeatArgs, args);
...
function stringReplace(args) {
    const [string, substr, newSubstr] = valueArgsValidate('stringReplace', stringReplaceArgs, args);
...
function stringSlice(args) {
    const [string, begin, endArg] = valueArgsValidate('stringSlice', stringSliceArgs, args);
    const end = endArg !== null ? endArg : string.length;
    if (begin > string.length) {
        valueArgsError('begin', begin);
    }
...
function stringSplit(args) {
    const [string, separator] = valueArgsValidate('stringSplit', stringSplitArgs, args);
...
function stringStartsWith(args) {
    const [string, search] = valueArgsValidate('stringStartsWith', stringStartsWithArgs, args);
...
function stringTrim(args) {
    const [string] = valueArgsValidate('stringTrim', stringTrimArgs, args);
    return string.trim();
}
...
function stringUpper(args) {
    const [string] = valueArgsValidate('stringUpper', stringUpperArgs, args);
...
function systemGlobalGet(args, options) {
    const [name, defaultValue] = valueArgsValidate('systemGlobalGet', systemGlobalGetArgs, args);
...
function systemGlobalSet(args, options) {
    const [name, value] = valueArgsValidate('systemGlobalSet', systemGlobalSetArgs, args);
    const globals = (options !== null ? (options.globals ?? null) : null);
    if (globals !== null) {
        globals[name] = value;
    }
...
function systemPartial(args) {
    const [func, funcArgs] = valueArgsValidate('systemPartial', systemPartialArgs, args);
    if (funcArgs.length < 1) {
        valueArgsError('args', funcArgs);
    }
...
function urlEncode(args) {
    const [url, extra] = valueArgsValidate('urlEncode', urlEncodeArgs, args);
...
function urlEncodeComponent(args) {
    const [url, extra] = valueArgsValidate('urlEncodeComponent', urlEncodeComponentArgs, args);
